...
# Sirf in zip-code ranges wali listings rakhni hain. Baqi sab (chahe
# Switzerland k bahar ho ya bas in ranges se bahar ho) skip ho jayengi.
VALID_ZIP_RANGES = [
    (2150, 2205),
    (2208, 2299),
    (2420, 2713),
...
def _zip_in_valid_ranges(zip_code: int) -> bool:
    return any(lo <= zip_code <= hi for lo, hi in VALID_ZIP_RANGES)


def fetch_sitemap(url: str):
    """
    Ek gzipped sub-sitemap fetch/decompress/parse karta hai.
    Har <url> k liye image:geo_location check karta hai — geo_location ka
    pehla hissa (postal/zip code) nikal k VALID_ZIP_RANGES k against check
    karte hain. Agar zip kisi bhi range mein nahi aata (chahe wo Switzerland
    k bahar ho ya bas un allowed ranges se bahar ho), us listing ko skip
    kar dete hain. Agar geo_location na mile, listing ko keep kiya jata hai.
    ...
    for url_el in root.findall('ns:url', NAMESPACES):
        loc_el = url_el.find('ns:loc', NAMESPACES)
        if loc_el is None or not loc_el.text:
            continue
        loc = loc_el.text.strip()

        geo_els = url_el.findall('.//image:geo_location', NAMESPACES)
        zip_codes = []
        for geo_el in geo_els:
            if geo_el.text:
                # geo_location format: "3007, Bern, CH" ya "8702, Zollikon, ZH, CH"
                # -> zip code hamesha pehla comma-separated part hota hai
                first_part = geo_el.text.split(",")[0].strip()
                if first_part.isdigit():
                    zip_codes.append(int(first_part))

        if zip_codes:
            # Kam se kam ek valid zip mil jaye to listing rakh lete hain
            if not any(_zip_in_valid_ranges(z) for z in zip_codes):
    ...
    if skipped_out_of_range:
        print(f"  -> Skipped {skipped_out_of_range} listing(s) outside valid zip ranges.")
